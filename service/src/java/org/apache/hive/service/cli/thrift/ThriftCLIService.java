...
public abstract class ThriftCLIService extends AbstractService implements TCLIService.Iface, Runnable {
  ...
  public synchronized void init(HiveConf hiveConf) {
    ...
    String portString;
    // HTTP mode, 默认为binary, 执行else中的逻辑
    ...
    minWorkerThreads = hiveConf.getIntVar(ConfVars.HIVE_SERVER2_THRIFT_MIN_WORKER_THREADS); // 默认值为5
    maxWorkerThreads = hiveConf.getIntVar(ConfVars.HIVE_SERVER2_THRIFT_MAX_WORKER_THREADS); // 默认值为500
    super.init(hiveConf);
  }
  ...
  public synchronized void start() {
    ...
    if (!isStarted && !isEmbedded) {
      // 这里的this实际是ThriftBinaryCLIService，即启动ThriftBinaryCLIService线程，进入ThriftBinaryCLIService的run方法
