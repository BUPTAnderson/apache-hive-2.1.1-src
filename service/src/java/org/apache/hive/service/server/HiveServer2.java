...
public class HiveServer2 extends CompositeService {
  ...
  public synchronized void init(HiveConf hiveConf) {
    ...
    // 实例化cliService，该实例用于把用户请求转化并传递给Driver
    ...
    if (isHTTPTransportMode(hiveConf)) {
      ...
      // ThriftCLIService是HiveServer2的thrift服务端, JDBC客户端这边通过thrift发过来的请求将全部落入这个对象进行处理，从他的类定义可以看到标准的thrift用法。
      // 无论是哪个ThriftCLIService，都传入了CLIService的引用，thrift只是一个封装。也就说ThriftCLIService负责处理通信, CLIService 负责业务处理。
    ...
    // Setup web UI
    // 启动web UI，该web UI用于查看正在进行的Hive任务，默认端口10002
    try {
      int webUIPort =
  ...
  private static void startHiveServer2() throws Throwable {
    ...
    while (true) {
      ...
      try {
        ...
        // 初始化，主要是获取了一些配置参数，并且告诉程序，要启动这些服务CLIService、thriftCLIService 、webServer
        server.init(hiveConf);
        // 启动，做了两件事情，就是调用父类start方法，启动web服务
        server.start();
        ...
        if (++attempts >= maxAttempts) { //如果抛出异常，并尝试启动超过了配置的最大尝试次数，抛出错误，启动失败
          throw new Error("Max start attempts " + maxAttempts + " exhausted", throwable);
        } else {
          LOG.warn("Error starting HiveServer2 on attempt " + attempts
              + ", will retry in 60 seconds", throwable);
          try {
            Thread.sleep(60L * 1000L);  //60秒后再次尝试启动
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      // oproc会对传入的启动参数进行解析，最后会返回一个ServerOptionsProcessorResponse实例，该实例中传入了一个ServerOptionsExecutor对象
      // 如果传入的是--help参数，对应的是HelpOptionExecutor实例，如果传入的是--deregister，对应的是DeregisterOptionExecutor实例，其它是StartOptionExecutor
      ...
      // 启动HiveServer2的部分，这里ServerOptionsExecutor接口有三个实现类
      // DeregisterOptionExecutor:将HiveServer2实例从zookeeper中移除
      // HelpOptionExecutor：打印help参数
      // StartOptionsExecutor：启动HiveServer2服务，传入启动参数正确，正常启动的话调用StartOptionExecutor的execute()方法
  ...
  static class ServerOptionsProcessor {
    ...
    ServerOptionsProcessorResponse parse(String[] argv) {
      try {
        ...
        // Process --hiveconf, 示例： -hiveconf hive.root.logger=DEBUG,console -hiveconf hive.security.authorization.enabled=true
