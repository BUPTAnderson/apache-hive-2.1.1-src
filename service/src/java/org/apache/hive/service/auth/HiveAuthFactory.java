...
public class HiveAuthFactory {
  ...
  public HiveAuthFactory(HiveConf conf) throws TTransportException {
    this.conf = conf;
    transportMode = conf.getVar(HiveConf.ConfVars.HIVE_SERVER2_TRANSPORT_MODE); // 默认值binary
    authTypeStr = conf.getVar(HiveConf.ConfVars.HIVE_SERVER2_AUTHENTICATION); // 默认值NONE
    ...
    if (authTypeStr == null) {
      if ("http".equalsIgnoreCase(transportMode)) {
        ...
        authTypeStr = AuthTypes.NONE.getAuthName(); // authTypeStr = "NONE"
      }
    }
    // isSASLWithKerberizedHadoop默认是false
